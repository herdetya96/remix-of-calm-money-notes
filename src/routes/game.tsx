...
function GasStationGame() {
  ...
  return (
    <div className="py-4">
      {/* Satu scene penuh — UI melayang di atas canvas */}
      <div className="relative h-[calc(100dvh-200px)] min-h-[440px] md:h-[calc(100dvh-150px)] rounded-2xl overflow-hidden shadow-[var(--shadow-xs)]">
        <ClientOnly
          fallback={
            <div className="absolute inset-0 bg-ink-40 flex items-center justify-center text-[13px] text-ink-400">
              Menyiapkan motor 3D...
            </div>
          }
        >
        ...
        <div className="absolute top-3 right-3 md:top-5 md:right-5 z-10 flex flex-col items-end gap-2">
          ...
          <div className="w-[176px] md:w-[200px] rounded-xl bg-ink-900/90 backdrop-blur p-3 space-y-2 shadow-[var(--shadow-sr)]">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-medium text-paper/40">Ronde {round}</span>
            </div>
            <PumpReadout
              label="Rupiah"
            ...
            <div className="border-t border-paper/10 pt-2 space-y-1">
              <div className="flex justify-between text-[11px]">
                ...
                <span className="font-semibold text-paper">{formatRupiah(order.targetMoney)}</span>
              </div>
              <div className="flex justify-between text-[11px]">
        ...
        {(phase === "ready" || phase === "filling") && (
          <div className="absolute bottom-4 md:bottom-6 inset-x-0 z-10 flex flex-col items-center gap-2 px-4">
            ...
            <p className="text-[11px] text-ink-500 text-center max-w-[420px] bg-paper/70 backdrop-blur rounded-full px-3 py-1">
              Tahan tombol merah atau tahan klik nozzle di tangki · geser area kosong untuk memutar
            </p>
          </div>
        )}
        ...
        {phase === "gameover" && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-ink-900/15 backdrop-blur-[2px] px-4">
            <div className="bg-paper rounded-2xl shadow-[var(--shadow-sr)] p-6 text-center space-y-3 max-w-[340px] w-full">
              ...
              <div className="text-[16px] font-semibold text-ink-900">Game Over</div>
              <div className="text-[13px] text-ink-500">
                Skor akhir: <span className="font-semibold text-ink-900">{score}</span>
                {score >= highscore && score > 0 && " — rekor baru! 🎉"}
              </div>
              <div className="text-[12px] text-ink-400">
                Pas (±Rp 100) = 100 poin. Kelebihan atau kurang &gt; Rp 1.500 = -1 nyawa.
              </div>
              <button
                type="button"
                onClick={restart}
